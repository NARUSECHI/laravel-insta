                            ...
                            @foreach ($user->following as $following)
                                <div class="row align-items-center mt-3">
                                    <div class="col-auto">
                                        <a href="{{route('profile.show',$following->following->id) }}">
                                            @if ($following->following->avatar)
                                                <img src="{{asset('storage/avatars/'.$following->following->avatar) }}" alt="{{$following->following->avatar}}" class="rounded-circle avatar-sm">
                                            @else
                                    ...
                                    <div class="col ps-0 text-truncate">
                                        <a href="{{ route('profile.show',$following->following->id) }}" class="text-decoration text-dark fw-bold">{{ $following->following->name }}</a>
                                    </div>
                                    <div class="col-auto text-end">
                                        @if ($following->following->id = Auth::user()->id)
                                            @if ($following->following->isFollowed())
                                                <form action="{{ route('follow.destroy',$follwing->following->id) }}" method="post">
                                                    @csrf
                                                    @method('DELETE')
